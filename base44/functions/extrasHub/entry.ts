  ...
  try {
    ...
    if(['booking_context','send_booking_message','rate_caterer','respond_booking','cancel_booking','delete_booking'].includes(body.action)){
      ...
      if(body.action==='booking_context'){
        ...
        const readAt=new Date().toISOString();const unread=(isExtra||isCaterer)?messages.filter(item=>item.sender_user_id!==user.id&&!item.read_at):[];if(unread.length)await base44.asServiceRole.entities.ExtraBookingMessage.bulkUpdate(unread.map(item=>({id:item.id,read_at:readAt,read_by_user_id:user.id})));const updatedMessages=messages.map(item=>unread.some(unreadItem=>unreadItem.id===item.id)?{...item,read_at:readAt,read_by_user_id:user.id}:item);
        ...
        return Response.json({messages:updatedMessages,average_rating:average,review_count:reviews.length,previous_missions:previousMissions.length,own_review:ownReview,can_rate:isExtra&&booking.status==='confirmed'&&booking.booking_date<=new Date().toISOString().slice(0,10)});
      }
