  ...
  try {
    ...
    if (['view', 'respond'].includes(body.action)) {
      ...
      if (body.action === 'view') {
        ...
        await base44.entities.OpportunityInteraction.create({ request_id: request.id, request_kind: body.kind, caterer_id: catererId, interaction_type: 'response', message });
      }
    ...
    const source = [...quotes.filter(item => item.transmission_consent && !['closed', 'cancelled'].includes(item.status)).map(item => ({ ...item, kind: 'quote' })), ...urgent.filter(item => item.transmission_consent && !['fulfilled', 'expired', 'cancelled', 'rejected'].includes(item.status)).map(item => ({ ...item, kind: 'urgent' }))];
