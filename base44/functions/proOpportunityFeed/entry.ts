...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    const summarize = item => {
      ...
      const itemDistance = Number.isFinite(profile?.latitude) && Number.isFinite(profile?.longitude) ? distanceKm(profile.latitude, profile.longitude, point.latitude, point.longitude) : null;
      ...
      const summary = { id: item.id, kind: item.kind, reference: item.reference, event_type: item.event_type, event_date: item.event_date, event_time: item.event_time, guest_count: item.guest_count, location: item.location, address: item.address, postal_code: item.postal_code, city: item.city, budget: item.budget, format: item.format, status: item.status, radius_km: item.radius_km, distance_km: itemDistance, viewed: mine.some(interaction => interaction.interaction_type === 'view'), responded: mine.some(interaction => interaction.interaction_type === 'response'), last_interaction_at: mine[0]?.created_date || '', view_count: requestInteractions.filter(interaction => interaction.interaction_type === 'view').length, response_count: requestInteractions.filter(interaction => interaction.interaction_type === 'response').length, ...point };
      return subscribed ? { ...summary, message: item.message, service_need: item.service_need } : summary;
    };
    const inServiceArea = item => user.role === 'admin' || item.distance_km == null || item.distance_km <= Math.min(Number(profile.service_radius_km || 50), Number(item.radius_km || 50));
    const items = activeSource.map(summarize).filter(Boolean).filter(inServiceArea).sort((a, b) => (a.distance_km ?? Infinity) - (b.distance_km ?? Infinity));
