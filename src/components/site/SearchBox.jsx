...
export default function SearchBox() {
  ...
  return <form onSubmit={submit} className="grid gap-3 rounded-2xl border bg-background/85 p-4 shadow-xl backdrop-blur md:grid-cols-5">
    ...
    <label className="text-xs font-semibold">Date <span className="font-normal text-muted-foreground">(optionnel)</span><input type="date" value={form.date} onChange={set('date')} className="mt-1 h-11 w-full rounded-lg border bg-background px-3 text-sm" /></label>
    <label className="text-xs font-semibold">Invités <span className="font-normal text-muted-foreground">(optionnel)</span><input min="1" type="number" value={form.guests} onChange={set('guests')} className="mt-1 h-11 w-full rounded-lg border bg-background px-3 text-sm" placeholder="80" /></label>
