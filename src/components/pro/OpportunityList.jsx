...
export default function OpportunityList({items,subscribed,selectedKey,onSelect}){
  ...
  return <aside className="max-h-[680px] space-y-3 overflow-y-auto pr-1">{ordered.map(item=>{const key=`${item.kind}:${item.id}`;return <OpportunityCard key={key} item={item} subscribed={subscribed} selected={key===selectedKey} onSelect={()=>onSelect(key)}/>})}{!items.length&&<div className="rounded-3xl border bg-card p-8 text-center text-muted-foreground">Aucune demande ne correspond à ces filtres.</div>}</aside>;
}