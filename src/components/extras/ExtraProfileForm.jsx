...
const defaults={first_name:'',last_name:'',email:'',phone:'',date_of_birth:'',address:'',postal_code:'',city:'',skills:[],experience_years:'',experience_details:'',languages:[],certifications:[],availability_notes:'',mobility_radius_km:'',transport:'',hourly_rate:'',photo_urls:[],available:true,display_last_name:false,display_email:false,display_phone:false,contact_preference:'site'};
export default function ExtraProfileForm({initial,user,onSaved}) {
  ...
  return <form onSubmit={submit} className="space-y-5 text-sm font-semibold">
    ...
    <div className="grid gap-4 sm:grid-cols-[1fr_130px]"><label>Adresse <span className="font-normal text-muted-foreground">(privée, facultative)</span><input value={data.address||''} onChange={set('address')} className="mt-1 h-11 w-full rounded-xl border bg-background px-3"/></label><label>Code postal<input value={data.postal_code||''} onChange={set('postal_code')} className="mt-1 h-11 w-full rounded-xl border bg-background px-3"/></label><label className="sm:col-span-2">Ville<input value={data.city||''} onChange={set('city')} className="mt-1 h-11 w-full rounded-xl border bg-background px-3"/></label></div>
    <ExtraWorkFields data={data} set={set}/><ExtraPrivacyFields data={data} set={set}/>
    ...
    {error&&<p className="rounded-xl bg-destructive/10 p-3 text-destructive">{error}</p>}<button disabled={busy} className="w-full rounded-xl bg-primary px-5 py-3 font-bold text-primary-foreground disabled:opacity-50">{busy?'Enregistrement…':'Enregistrer mon profil'}</button>
  </form>;
}