...
const defaults={first_name:'',last_name:'',email:'',phone:'',date_of_birth:'',address:'',postal_code:'',city:'',skills:[],experience_years:'',experience_details:'',languages:[],certifications:[],availability_notes:'',availability_dates:[],availability_period:'day',mobility_radius_km:'',transport:'',hourly_rate:'',photo_urls:[],available:true,display_last_name:false,display_email:false,display_phone:false,contact_preference:'site'};
export default function ExtraProfileForm({initial,user,onSaved}) {
  ...
  return <form onSubmit={submit} className="space-y-5 text-sm font-semibold">
    ...
    <ExtraWorkFields data={data} set={set}/><ExtraAvailabilityCalendar value={data.availability_dates} period={data.availability_period} onChange={availability_dates=>setData(x=>({...x,availability_dates}))} onPeriodChange={availability_period=>setData(x=>({...x,availability_period}))}/><ExtraPrivacyFields data={data} set={set}/>
