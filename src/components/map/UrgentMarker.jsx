...
export default function UrgentMarker({item,subscribed,selected,onSelect}) {
  const date = new Date(`${item.event_date}T12:00:00`).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' });const marker=useRef(null);const map=useMap();const navigate=useNavigate();const icon=useMemo(()=>buildIcon(selected),[selected]);const respond=event=>{event.stopPropagation();if(onSelect)onSelect();else navigate(`/opportunites-pro?opportunity=urgent:${item.id}`)};
