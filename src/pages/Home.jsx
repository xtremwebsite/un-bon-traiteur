...
const categories = ['Mariage','Anniversaire','Cocktail','Brunch','Entreprise','Réception privée'];
export default function Home() {
  return <><Header/><main>
    ...
    <section className="mx-auto max-w-7xl px-4 py-14"><h2 className="font-heading text-3xl font-bold">Pour chaque moment à célébrer</h2><div className="mt-7 grid grid-cols-2 gap-3 md:grid-cols-6">{categories.map((x)=><Link key={x} to={`/recherche?event=${x}`} className="rounded-xl border bg-card p-4 text-center font-semibold shadow-sm"><span className="mb-2 block text-2xl">✦</span>{x}</Link>)}</div></section>
    <NearbyCaterers/>
