...
const initial = () => {
  ...
  return { city: p.get('location') || '', event: p.get('event') || '', format: p.get('format') || '', maxPrice: '', rating: '', verified: false, geo: null, radius: '50' };
};
...
export default function Search() {
  ...
  useEffect(() => { Promise.all([base44.entities.CatererProfile.filter({ published: true }, '-updated_date', 100), base44.entities.EventType.filter({ active: true }, 'display_order', 100), base44.entities.Format.filter({ active: true }, 'display_order', 100)]).then(([profiles,eventItems,formatItems]) => { setItems(profiles); setEvents(eventItems.map(x => x.name)); setFormats(formatItems.map(x => x.name)); }).finally(() => setLoading(false)); }, []);
  const shown = useMemo(() => {
    const city = filters.city.trim().toLowerCase();
    const result = items.filter(x => (!city || `${x.city} ${x.postal_code || ''}`.toLowerCase().includes(city)) && (!filters.event || (x.event_types || []).includes(filters.event)) && (!filters.format || (x.formats || []).includes(filters.format)) && (!filters.maxPrice || (typeof x.price_from_per_person === 'number' && x.price_from_per_person <= Number(filters.maxPrice))) && (!filters.rating || (x.google_rating || 0) >= Number(filters.rating)) && (!filters.verified || x.verified) && (!filters.geo || (x.latitude && x.longitude && distance(filters.geo, x) <= Number(filters.radius))));
