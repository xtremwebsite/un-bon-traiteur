...
export default function Admin() {
  ...
  return <AdminGuard><div className="min-h-screen bg-primary/5"><AdminHeader/><main className="mx-auto max-w-7xl px-4 py-8"><div className="rounded-[2rem] border bg-card/90 p-6 shadow-xl backdrop-blur-xl"><p className="text-sm font-bold text-destructive">Pilotage de la plateforme</p><h1 className="mt-1 font-heading text-4xl font-bold">Back-office administrateur</h1><p className="mt-2 text-muted-foreground">Supervisez l’activité depuis un espace unique et structuré.</p></div><div className="mt-6 grid items-start gap-6 lg:grid-cols-[240px_minmax(0,1fr)]"><AdminTabs active={active} onChange={setActive}/><section className="min-w-0">{loading ? <div className="h-52 animate-pulse rounded-3xl bg-muted"/> : <>{active === 'overview' && <AdminStats stats={stats}/>} {active === 'professionals' && <CatererTable items={data.caterers} users={data.users} onUpdate={(id,payload) => update('CatererProfile',id,payload)} onUpdateUser={(id,payload) => update('User',id,payload)}/>} {active === 'users' && <UserTable items={data.users.filter(user => user.role !== 'admin' && !data.caterers.some(profile => profile.profile_origin === 'owner' && profile.created_by_id === user.id))} currentUserId={data.me?.id} onUpdate={(id,payload) => update('User',id,payload)}/>} {active === 'subscriptions' && <PlanTable items={data.plans} onUpdate={(id,payload) => update('SubscriptionPlan',id,payload)}/>} {active === 'catalogs' && <CatalogManager/>} {active === 'requests' && <div className="space-y-6"><QuoteTable items={data.quotes} onUpdate={(id,payload) => update('QuoteRequest',id,payload)}/><UrgentTable items={data.urgent} onUpdate={(id,payload) => update('UrgentRequest',id,payload)}/></div>} {active === 'reviews' && <ReviewTable items={data.reviews} caterers={data.caterers} onUpdate={(id,payload) => update('Review',id,payload)}/>}</>}</section></div></main></div></AdminGuard>;
}