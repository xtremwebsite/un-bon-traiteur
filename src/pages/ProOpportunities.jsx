...
export default function ProOpportunities(){
  ...
  const items=useMemo(()=>sourceItems.map(item=>({...item,distance:distance(origin[0],origin[1],Number(item.latitude),Number(item.longitude))})).filter(item=>(filters.kind==='all'||item.kind===filters.kind)&&(filters.eventType==='all'||item.event_type===filters.eventType)&&item.distance<=filters.radius&&`${item.location} ${item.event_type}`.toLowerCase().includes(filters.search.toLowerCase())),[sourceItems,filters,origin[0],origin[1]]);
