...
export default function ProOpportunities(){
  const[data,setData]=useState({items:[],subscribed:false,profile:null});const[loading,setLoading]=useState(true);const[filters,setFilters]=useState({search:'',kind:'all',eventType:'all',radius:50});
  useEffect(()=>{base44.functions.invoke('proOpportunityFeed',{}).then(response=>setData(response.data)).finally(()=>setLoading(false))},[]);
  const origin=Number.isFinite(data.profile?.latitude)?[data.profile.latitude,data.profile.longitude]:[46.6,2.2];
  const items=useMemo(()=>data.items.map(item=>({...item,distance:distance(origin[0],origin[1],item.latitude,item.longitude)})).filter(item=>(filters.kind==='all'||item.kind===filters.kind)&&(filters.eventType==='all'||item.event_type===filters.eventType)&&item.distance<=filters.radius&&`${item.location} ${item.event_type}`.toLowerCase().includes(filters.search.toLowerCase())),[data.items,filters,origin[0],origin[1]]);
